...
fn fragmentMain( input: VertexOutput) -> @location(0) vec4f {
    ...
    let c = (input.uv + 1.) / 2.;
    let v = input.state;
    let color:vec3f = hsv2rgb(vec3f( abs(v.y-v.x) , 1., v.y + .05 ));
...
fn computeMain(@builtin(global_invocation_id) cell: vec3u) {
    ...
    let params = array<vec2<f32>,9>(
        ...
        vec2(0.016, 0.051), // spots and loops
        vec2(0.062, 0.0609), // U-SKATE
        vec2(0.039, 0.058), // holes
        vec2(0.022, 0.059) // microbes
    );
    ...
    debug[0] = vec4f( vec2f(pos), current[index].x, current[index].y);   
}
