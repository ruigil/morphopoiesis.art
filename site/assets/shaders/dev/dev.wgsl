...
fn fragMain(input : VertexOutput) -> @location(0) vec4<f32> {
  ...
  return vec4( vec3(  input.state.y , 0, 0 ) ,1.0) ;
  //return vec4( 1. );
}
...
fn computeAgents(@builtin(global_invocation_id) id : vec3<u32>) {
    ...
    // if the next position is free move to it, only one particle per pixel.
      ...
      agents[i].pos = pos;
      trailMapB[ current.y * u32(params.size.x) + current.x ] = vec2<f32>(0., 0.);
      trailMapB[ next.y * u32(params.size.x) + next.x ] = vec2<f32>(0., 1.);
